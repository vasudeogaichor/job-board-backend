...
export default class UserController {
  static async getProfile(
    ...
    console.log('topicMastery - ', topicMastery)
    ...
    // Recent number of quizzes by topic
    ...
    // Performance by topic (weak/strong topics)
    const performanceByTopic = await QuizAttempt.aggregate([
      { $match: { user: user._id, completedAt: { $ne: null } } },
      { $unwind: "$topicPerformance" },
      {
        $group: {
          _id: "$topicPerformance.topic",
          averageScore: { $avg: "$topicPerformance.score" },
          minScore: { $min: "$topicPerformance.score" },
          maxScore: { $max: "$topicPerformance.score" },
        },
      },
    ]);

    // Structure topic stats
    const topicStats = performanceByTopic.map((topic: any) => ({
      topic: topic._id,
      averageScore: topic.averageScore,
      minScore: topic.minScore,
      maxScore: topic.maxScore,
      recentQuizzes:
        quizzesByTopic.find((q) => q._id === topic._id)?.count || 0,
    }));
    ...
    return Response.success({
      ...
      topicMastery: topicMastery.reduce((acc: ITopicMastery, topic: any) => {
        console.log('type - ', topic.averageScore)
